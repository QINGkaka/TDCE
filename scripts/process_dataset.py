...
def process_data(name):
    ...
    if info['file_type'] == 'csv':
        # TDCE: Handle OpenML downloaded LAW dataset (auto-detect structure)
        if name == 'law':
            # Try to read with header first, auto-detect column names
            try:
                data_df = pd.read_csv(data_path, header=0, low_memory=False)
                # If column_names is empty, use actual column names from dataframe
                if not info.get('column_names') or len(info.get('column_names', [])) == 0:
                    info['column_names'] = data_df.columns.tolist()
                    # Auto-detect column types
                    column_info = {}
                    for col in data_df.columns:
                        if data_df[col].dtype in ['float64', 'float32', 'int64', 'int32']:
                            column_info[col] = "float"
                        else:
                            column_info[col] = "str"
                    info['column_info'] = column_info
                    
                    # Auto-detect target column (usually last column or named 'pass_bar')
                    if 'pass_bar' in data_df.columns:
                        info['target_col'] = 'pass_bar'
                    else:
                        # Use last column as target
                        info['target_col'] = data_df.columns[-1]
                    
                    # Auto-detect immutable features (gender, race-related columns)
                    immutable_cols = []
                    for col in data_df.columns:
                        col_lower = col.lower()
                        if col_lower in ['gender', 'sex'] or 'race' in col_lower:
                            immutable_cols.append(col)
                    info['immutable'] = immutable_cols
            except:
    ...
    if task_type == 'binclass':
        ...
        # Count unique classes from target
        unique_classes = len(set(y_train.flatten()))
        info['n_classes'] = unique_classes
...
if __name__ == "__main__":
    ...
    if args.dataname:
        ...
        # for name in ['adult', 'adult', 'default', 'lending-club', 'gmc']:#, 'adult', 'default', 'lending-club', 'gmc']:#, 'default', 'shoppers', 'magic', 'beijing', 'news']:    
        for name in ['spirals']:
            process_data(name)
